...
import { ensureFileSync } from 'https://deno.land/std@0.134.0/fs/ensure_file.ts';
import * as colours from "https://deno.land/std@0.134.0/fmt/colors.ts";
...
class Logger {
  ...
  public log(level: Logger.LogLevel, msg: string): boolean {
    ...
    if (logIt) {
      ...
      // No colouring
      const strippedLog = new TextEncoder().encode(
        // Label
        '[' + this.label() + '] ' +
        // Name (if any)
        (this.name === undefined ? '' : '[' + this.name + '] ') +
